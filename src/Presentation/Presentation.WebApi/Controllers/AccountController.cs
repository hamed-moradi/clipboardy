...
namespace Presentation.WebApi.Controllers
{
    public class AccountController : BaseController
    {
        ...
        [HttpGet, Route("CheckRememberMe")]
        public async Task<IActionResult> CheckRememberMe()
        {
            try
            {
                var id = HttpContext.User.Claims.FirstOrDefault(x => x.Type == "id").Value;
                var result = await _accountService.GetAccountService(Convert.ToInt32(id)).ConfigureAwait(true);
                return Respond(result);
            }
            catch (Exception ex)
            {
